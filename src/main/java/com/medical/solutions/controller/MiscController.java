...
import com.medical.solutions.factory.MiscFactory;
...
public class MiscController {
	...
	public Response sendEmail_SingleTO(@RequestBody com.medical.solutions.request.MiscEmailRequest miscEmailRequest) {

		Email email = new Email();
		try {
			BeanUtils.copyProperties(miscEmailRequest, email);
		} catch (BeansException beansException) {
			throw new BadRequestException("Email address is not correct", beansException);
		}
	...
	public Response emailSingleTO_MultipleCC(
			@RequestBody com.medical.solutions.request.MiscEmailRequest miscEmailRequest) {
	 ...
	 * List<Contact> getAll(@RequestBody Contact contact) { return
	 * miscFactory.getMiscService().getAll(contact); }
	 */
	...
	/*
	 * @RequestMapping("/advertiseDoctor") public List<AdvertiseDoctor> test() {
	 * AdvertiseDoctor advertiseDoctor = new AdvertiseDoctor();
	 * advertiseDoctor.setId(1); advertiseDoctor.setDoctorName("Mahima");
	 * advertiseDoctor.setClinicName("ABC");
	 * advertiseDoctor.setClinicAddress("Delhi"); List<AdvertiseDoctor> response
	 * = miscFactory.getMiscService() .getDoctorByDate(advertiseDoctor); return
	 * response;
	 * 
	 * }
	 */
