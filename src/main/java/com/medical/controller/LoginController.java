...
import com.medical.doctor.factory.PatientLoginFactory;
...
import com.medical.patient.request.PatientLoginRequest;
...
public class LoginController {
	...
	@ApiResponses(value = { @ApiResponse(code = 400, message = "Fields are with validation errors"),
			@ApiResponse(code = 201, message = "") })
	public LoginResponse patientLoginValidate(@RequestBody PatientLoginRequest patientLoginRequest) {
		...
		if(!StringUtils.isEmpty(patientLogin) && !StringUtils.isEmpty(patientLogin.getUsername()) && !StringUtils.isEmpty(patientLogin.getPassword())){
			return new LoginResponse(patientLoginFactory.getPatientLoginService().validateLogin(patientLogin));	
		}else{
			throw new BadRequestException(
					"Login Username and Password should not be blank");
		}
